...
**Button & UX Refinement (Sept 30, 2025)**: Upgraded "Affirm me" button to burnt apricot palette (#C98B5F base, #B3744B hover, #905936 outline) with richer color that stands out against dark florals. System sans font weight 600, sentence case with 0.25px letter-spacing. Enhanced with subtle top highlight gradient, soft shadow, and intentional interaction states. Controls bar improved with raised opacity backdrop (rgba 0,0,0,0.18) for better stability. Fixed FAQ text selection issue with user-select: none. Optimized mobile controls (@480px) with vertical stack layout (flex-direction: column) for better alignment.

**Glass Button Redesign (Sept 30, 2025)**: Transformed primary button to slick glass design with transparent background (rgba 201,139,95,0.04 peach tint), 7px backdrop blur, 1px semi-opaque white border, and thin gold inner stroke accent. Text updated to "Affirm Me" (title case), near-white color (rgba 255,255,255,0.95), 0.3px letter-spacing. Enhanced with subtle top highlight (38% height), soft shadow, hover brightness (+3%), and focus rings (inner light + outer gold). Controls bar background removed for cleaner aesthetic. Mobile controls stack vertically with full-width elements and centered alignment.
